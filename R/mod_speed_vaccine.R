  ...
  tagList(
    ...
    card(
      ...
      p(class = "text-muted small mb-1",
        "Top speed is each athlete's best recorded in a logged session, so a
         new personal best raises their 90% target automatically. Bars use
         the same traffic light as the table: green ≤5 days, gold 6-7,
         red >7 or never."),
      uiOutput(ns("no_speed_note")),
...
mod_speed_vaccine_server <- function(id, vaccine, data = NULL) {
  moduleServer(id, function(input, output, session) {

    # Athletes with no usable speed reading are excluded from the traffic
    # light rather than shown as a misleading colour -- say so explicitly.
    output$no_speed_note <- renderUI({
      if (is.null(data)) return(NULL)
      g <- data()$gps
      no_speed <- g |>
        distinct(athlete_id, athlete_name, vmax) |>
        filter(is.na(vmax) | vmax <= 0)
      if (nrow(no_speed) == 0) return(NULL)
      p(class = "small mb-1", style = paste0("color:", AMS_COLORS$gold),
        sprintf("%d athlete%s excluded — no valid speed recorded yet: %s",
                nrow(no_speed), if (nrow(no_speed) == 1) "" else "s",
                paste(no_speed$athlete_name, collapse = ", ")))
    })
